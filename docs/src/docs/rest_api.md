...
| `r_[field name]` | To filter using a range of possible values (for number, decimal, date, and geospatial fields). Separate min and max values by "to" (for number, decimal, and date fields, if either min or max are omitted, range will just be "less than or equal to" or "greater than or equal to" respectively). | Number/Decimal/Date:<br />[endpoint url]?r_size=6to10<br /><br />Geospatial (bounding box):<br />[endpoint url]?r_location=[lon],[lat]to[lon],[lat] 
